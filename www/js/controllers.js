...
angular.module('app.controllers', ['ionic-material'])
    ...
    $scope.loginEmail = function(formName,cred) {
      ...
      if(formName.$valid) {  // Check if the form data is valid or not
          ...
          firebase.auth().signInWithEmailAndPassword(cred.email,cred.password).then(function(result) {
            ...
            function(error) {
              sharedUtils.hideLoading();
              sharedUtils.showAlert("Pesan","Gagal Masuk");
            }
        );

      }else{
        sharedUtils.showAlert("Pesan","Masukkan data dengan benar");
      }
...
.controller('menuCtrl', function($scope,$rootScope,$ionicSideMenuDelegate,fireBaseData,$state,
  ...
  $scope.$on('$ionicView.enter', function(ev) {
   ...
   $timeout(function(){
          var refSlider = new Firebase("https://shop-4eac7.firebaseio.com/slider")
...
.controller('settingsCtrl', function($scope,$rootScope,fireBaseData,$firebaseObject,
    ...
    $scope.addManipulation = function(edit_val) {  // Takes care of address add and edit ie Address Manipulator
      ...
      if(edit_val!=null) {
        ...
        var title="Ubah Alamat";
        var sub_title="Masukkan Alamat Anda";
      }
      else {
      ...
      var addressPopup = $ionicPopup.show({
        template: '<input type="text"   placeholder="Nama Penerima"  ng-model="data.nickname"> <br/> ' +
                  '<input type="text"   placeholder="Alamat" ng-model="data.address"> <br/> ' +
                  '<input type="number" placeholder="Kode Pos" ng-model="data.pin"> <br/> ' +
                  '<input type="number" placeholder="No Handphone" ng-model="data.phone">',
        title: title,
        subTitle: sub_title,
        scope: $scope,
        buttons: [
    ...
    $scope.save= function (extras,editable) {
      ...
      if(editable.password!="" && editable.password!=null  ){
        ...
        sharedUtils.showAlert("Account","Password berubah");
      }
...
.controller('checkoutCtrl', function($scope,$rootScope,sharedUtils,$state,$firebaseArray,$firebaseObject,
    ...
    $scope.pay=function(address,payment){
      ...
      if(address==null || payment==null){
        ...
        sharedUtils.showAlert("Info", "Pesanan Anda Berhasil Disimpan Tunggu Konfirmasi");
    ...
    $scope.addManipulation = function(edit_val) {  // Takes care of address add and edit ie Address Manipulator
      ...
      if(edit_val!=null) {
        ...
        $scope.data = {};    // For adding new address
        var title="Tambah Alamat";
        var sub_title="Masukkan Alamat Baru Anda";
      }
      // An elaborate, custom popup
      var addressPopup = $ionicPopup.show({
        template: '<input type="text"   placeholder="Nama Penerima"  ng-model="data.nickname"> <br/> ' +
